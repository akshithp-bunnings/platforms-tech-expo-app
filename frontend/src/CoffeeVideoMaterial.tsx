...
const DEFAULT_THUMB = '/videos/default-thumb.jpg';
...
export const CoffeeVideoMaterial = ({ videoSrc, thumbSrc, active = true }:
  ...
  const actualVideoSrc = videoSrc || DEFAULT_VIDEO;
  const actualThumbSrc = thumbSrc || DEFAULT_THUMB;
  
  const { videoElement, canPlay } = useVideoElement(actualVideoSrc, active, { debug: false });
  ...
  useFrame(() => {
    ...
    // Only set videoReady to true if video actually exists and can play
  ...
  // Use error handling with texture loading
  let thumbTexture;
  try {
    thumbTexture = useTexture(actualThumbSrc);
  } catch (error) {
    console.warn(`Failed to load texture: ${actualThumbSrc}`, error);
    thumbTexture = useTexture(DEFAULT_THUMB);
  }
